...
namespace Common {
    ...
    // Create a thread instance, set affinity, assign it a name, and
    // pass the function to be run on the thread and the arguments to the function.
            ...
            auto thread_body = [&] {
                if (core_id >= 0 && !setThreadCore(core_id)) {
                    std::cerr << "Failed to set core affinity for " <<
                        name << " " << pthread_self() << " to " << core_id << std::endl;
                    failed = true;
                    return;
                }
